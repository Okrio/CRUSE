...
import train_base.loss as loss
from train_base.utils import initialize_module
...
def entry(rank, world_size, config, resume, only_validation):
    ...
    if rank == 0:
        ...
        init(os.path.join(config["meta"]["save_dir"], "train.log"),
             "train",
             slack_url=None)
    ...
    train_dataset = initialize_module(config["train_dataset"]["path"],
    ...
    valid_dataloader = DataLoader(dataset=initialize_module(
        config["validation_dataset"]["path"],
        args=config["validation_dataset"]["args"]),
                                  num_workers=0,
                                  batch_size=1)

    model = initialize_module(config["model"]["path"],
    ...
    loss_function = getattr(
        loss,
        config["loss_function"]["name"])(**config["loss_function"]["args"])
    trainer_class = initialize_module(config["trainer"]["path"],
                                      initialize=False)

    trainer = trainer_class(dist=dist,
                            rank=rank,
                            config=config,
                            resume=resume,
