...
from train_base.acoustics.feature import stft, istft, mc_stft
from train_base.utils import initialize_module, prepare_device, prepare_empty_dir
...
class BaseInferencer:
    def __init__(self, config, checkpoint_path, output_dir):
        ...
        self.model, epoch = self._load_model(config["model"], checkpoint_path,
                                             self.device)
        ...
        self.torch_istft = partial(istft,
                                   ...
                                   hop_length=self.hop_length,
                                   win_length=self.win_length)
        self.torch_mc_stft = partial(mc_stft,
                                     n_fft=self.n_fft,
                                     hop_length=self.hop_length,
                                     win_length=self.win_length)
    ...
    def _load_dataloader(dataset_config):
        dataset = initialize_module(dataset_config["path"],
    ...
    def _unfold(input, pad_mode, n_neighbor):
        ...
        """
        assert input.dim(
        ) == 4, f"The dim of input is {input.dim()}, which should be 4."
        ...
        output = functional.pad(output, [0, 0, n_neighbor, n_neighbor],
                                mode=pad_mode)
        output = functional.unfold(output, (sub_band_n_freqs, n_frames))
        assert output.shape[
            -1] == n_freqs, f"n_freqs != N (sub_band), {n_freqs} != {output.shape[-1]}"

        # 拆分 unfold 中间的维度
        output = output.reshape(batch_size, n_channels, sub_band_n_freqs,
                                n_frames, n_freqs)
        output = output.permute(0, 4, 1, 2, 3).contiguous(
        )  # permute 本质上与  reshape 可是不同的 ...，得到的维度相同，但 entity 不同啊
        return output

    @staticmethod
    def _load_model(model_config, checkpoint_path, device):
        model = initialize_module(model_config["path"],
    ...
    def multi_channel_mag_to_mag(self, noisy, inference_args=None):
        ...
        mixture_mag = (mixture_stft_coefficients.real**2 +
                       mixture_stft_coefficients.imag**2)**0.5
        ...
        reference_channel_stft_coefficients = mixture_stft_coefficients[:, 0,
                                                                        ...]
        noisy_phase = torch.atan2(reference_channel_stft_coefficients.imag,
    ...
    def __call__(self):
        ...
        assert inference_type in dir(
            self), f"Not implemented Inferencer type: {inference_type}"
        ...
        for noisy, name in tqdm(self.dataloader, desc="Inference"):
            ...
            t1 = time.time()
            enhanced = getattr(self, inference_type)(noisy.to(self.device),
                                                     inference_args)
            t2 = time.time()
            ...
            enhanced = np.int16(0.8 * amp * enhanced /
                                np.max(np.abs(enhanced)))

            # cal rtf
            rtf = (t2 - t1) / (len(enhanced) * 1.0 /
                               self.acoustic_config["sr"])
            print(f"{name}, rtf: {rtf}")

            # clnsp102_traffic_248091_3_snr0_tl-21_fileid_268 => clean_fileid_0
            # name = "clean_" + "_".join(name.split("_")[-2:])
            sf.write(self.enhanced_dir / f"{name}.wav",
                     enhanced,
