...
def load_checkpoint(checkpoint_path, device):
    ...
    assert ext in (
        ".pth",
        ".tar"), "Only support ext and tar extensions of l1 checkpoint."
    model_checkpoint = torch.load(os.path.abspath(
    ...
    def merge(older_dict, newer_dict):
        for new_key in newer_dict:
            ...
            if isinstance(older_dict[new_key], dict):
                older_dict[new_key] = merge(older_dict[new_key],
                                            newer_dict[new_key])
            else:
